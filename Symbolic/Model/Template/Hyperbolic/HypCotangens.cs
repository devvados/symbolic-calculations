...
namespace Symbolic.Model.Template.Hyperbolic
{
    class HypCotangens : Function
    {
        ...
        {
            return MathNet.Numerics.Trig.Coth(val);
        }
        ...
        {
            return 1 - Cth(InnerF) ^ 2 * InnerF.Derivative();
        }
        ...
        {
            return $"(exp({InnerF}) + exp({(-1) * InnerF}))/(exp({InnerF}) - exp({(-1) * InnerF}))";
        }
